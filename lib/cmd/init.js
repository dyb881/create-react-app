...
module.exports = () => {
  co(function*() {
    ...
    let type = yield prompt('项目类型，不填为默认(default/mobile/admin): ');

    if (!type) {
      type = 'default';
    } else if (!['mobile', 'admin'].includes(type)) {
      console.log(err);
      tip.fail('请填写正确的项目类型!');
      process.exit();
    }
    ...
    exec(cmdStr, err => {
      ...
      if (type === 'mobile') {
        info += ' @dyb881/auto-rem @dyb881/tab-bar';
      }

      if (type === 'admin') {
        info += ' react-media react-json-view @dyb881/mock-server';
      }
