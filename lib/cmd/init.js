...
module.exports = () => {
  ...
  }).then(({ name, device, template = 'default' }) => {
    if (!name) {
      if (template === 'default') {
    ...
    exec(cmdStr, err => {
      ...
      if (template !== 'default') {
        if (template === 'admin') {
          ...
          packageJson.scripts.mock = 'node src/apis/mock-serve.js';
          fs.writeJsonSync(`${name}/package.json`, packageJson, { spaces: 2 });
      ...
      const packages = ['lodash', '@types/lodash'];

      if (device === 'pc') {
        packages.push('antd@next', 'dayjs');
      ...
      if (device === 'mobile') {
        packages.push('antd-mobile', 'rc-field-form', 'whatwg-fetch', 'core-js', 'ua-parser-js', '@types/ua-parser-js');
      ...
      tip.info(`cd ${name}`);
      tip.info('执行以下命令安装依赖');
      tip.info(`npm i && npm i ${packages.join(' ')}`);
      tip.info('or');
      tip.info(`yarn && yarn add ${packages.join(' ')}`);
